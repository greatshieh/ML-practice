...
def ChiMerge(df, variable, flag, confidenceVal=3.841, bin=10, sample=False):
    '''
    ...
    regroup[
        'negative_class'] = regroup['total_num'] - regroup['positive_class']  # 统计需分箱变量每个值负样本数
...
#对相邻两个区间进行卡方值计算
    ...
    while (1):
        ...
        np_regroup[chi_min_index,
                   1] = np_regroup[chi_min_index, 1] + np_regroup[chi_min_index
                                                                  + 1, 1]
        np_regroup[chi_min_index,
                   2] = np_regroup[chi_min_index, 2] + np_regroup[chi_min_index
                                                                  + 1, 2]
    ...
    for i in np.arange(np_regroup.shape[0]):
        if i == 0:
            ...
            interval_list.append(np_regroup[i, 0])
        elif i == np_regroup.shape[0] - 1:
            x = str(np_regroup[i - 1, 0]) + '+'
            interval_list.append(np_regroup[i-1, 0])
        else:
    ...
    return result_data, interval_list