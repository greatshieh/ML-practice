...
class MDLP_Discretizer(object):
    def __init__(self,
        ...
        if not isinstance(
                dataset,
                pd.core.frame.DataFrame):  # class needs a pandas dataframe
        ...
        if features:
            self._features = [
                f for f in features if f in self._data_raw.columns
            ]  # check if features in dataframe
            missing = set(features) - set(
                self._features)  # specified columns not in dataframe
            if missing:
                print(
                    'WARNING: user-specified features %s not in input dataframe'
                    % str(missing))
        ...
        self._ignored_features = set(self._data_raw.columns) - set(
            self._features)
        ...
        self.apply_cutpoints(
            out_data_path=out_path_data, out_bins_path=out_path_bins)
    ...
    def MDLPC_criterion(self, data, feature, cut_point):
        ...
        cut_point_gain = cut_point_information_gain(
        ...
        entropy_left = entropy(
            data_left[self._class_name])  # entropy of partition
        entropy_right = entropy(data_right[self._class_name])
        delta = log(3**k, 2) - (k * partition_entropy) + (
            k_left * entropy_left) + (k_right * entropy_right)
    ...
    def feature_boundary_points(self, data, feature):
        ...
        data_partition = data.copy(deep=True)
        data_partition.sort_values(by=[feature], ascending=True, inplace=True)

        boundary_points = []

        #add temporary columns
        data_partition['class_offset'] = data_partition[
        ...
        data_partition['feature_offset'] = data_partition[feature].shift(
            1)  # column where first value is now second, and so forth
        data_partition['feature_change'] = (data_partition[feature] !=
                                            data_partition['feature_offset'])
        data_partition['mid_points'] = data_partition.loc[:, [
            feature, 'feature_offset'
        ]].mean(axis=1)

        potential_cuts = data_partition[data_partition['feature_change'] ==
                                        True].index[1:]
        sorted_index = data_partition.index.tolist()

        for row in potential_cuts:
            old_value = data_partition.loc[sorted_index[sorted_index.index(row)
                                                        - 1]][feature]
            new_value = data_partition.loc[row][feature]
            old_classes = data_partition[data_partition[feature] == old_value][
                self._class_name].unique()
            new_classes = data_partition[data_partition[feature] == new_value][
                self._class_name].unique()
    ...
    def compute_boundary_points_all_features(self):
        ...
        for attr in self._features:
            ...
            boundaries[attr] = self.feature_boundary_points(
                data=data_partition, feature=attr)
        return boundaries
    ...
    def boundaries_in_partition(self, data, feature):
        ...
        return set([
            x for x in self._boundaries[feature]
    ...
    def best_cut_point(self, data, feature):
        ...
        if not candidates:
            return None
        gains = [(cut, cut_point_information_gain(
            dataset=data,
            cut_point=cut,
            feature_label=feature,
    ...
    def single_feature_accepted_cutpoints(
            self, feature, partition_index=pd.DataFrame().index):
        '''
        ...
        data_partition = self._data.loc[partition_index,
                                        [feature, self._class_name]]
        ...
        cut_candidate = self.best_cut_point(
            data=data_partition, feature=feature)
        if cut_candidate == None:
            return
        decision = self.MDLPC_criterion(
            data=data_partition, feature=feature, cut_point=cut_candidate)
        ...
        if decision:
            ...
            left_partition = data_partition[data_partition[feature] <=
                                            cut_candidate]
            right_partition = data_partition[data_partition[feature] >
                                             cut_candidate]
            ...
            self.single_feature_accepted_cutpoints(
                feature=feature, partition_index=left_partition.index)
            self.single_feature_accepted_cutpoints(
                feature=feature, partition_index=right_partition.index)
    ...
    def apply_cutpoints(self, out_data_path=None, out_bins_path=None):
        ...
        for attr in self._features:
            if len(self._cuts[attr]) == 0:
                ...
                bin_labels = [
                    '%s_to_%s' % (str(cuts[i]), str(cuts[i + 1]))
                    for i in start_bin_indices
                ]
                bin_label_collection[attr] = bin_labels
                self._data[attr] = pd.cut(
                    x=self._data[attr].values,
                    bins=cuts,
                    right=False,
                    labels=bin_labels,
                    precision=6,
        ...
        if self._ignored_features:
            to_return = pd.concat(
                [self._data, self._data_raw[list(self._ignored_features)]],
                axis=1)
            to_return = to_return[
                self._data_raw.
                columns]  #sort columns so they have the original order
        else:
        ...
        if out_bins_path:
            with open(out_bins_path, 'w') as bins_file:
                bins_file.write(
                    'Description of bins in file: %s' % out_data_path)
                for attr in self._features:
                    bins_file.write('attr: %s\n\t%s' % (attr, ', '.join([
                        bin_label for bin_label in bin_label_collection[attr]
                    ])))
...
def main(**argv):
    ...
    for opt, value in parameters.items():
        if opt == 'in_path':
            data_path = value
    ...
    discretizer = MDLP_Discretizer(
        dataset=data,
        class_label=class_label,
        features=features,
        out_path_data=out_path_data,
        out_path_bins=out_path_bins)
...
argv = {
    'in_path': '../dataset/train_modified.csv',
    'out_path': 'result.csv',
    'features': 'MonthlyIncome',
    'return_bins': 0,
    'class_label': 'Attrition'
}
